...
### Motivation
The intuition for this project stems from a comparative observation of social media platforms. I observed that content on Twitter (X) often feels significantly more extreme in its emotional delivery compared to platforms like Weibo. This project explores how that intensity manifests in a political context and how it affects audience interaction.

### Key Findings
* **Intensity Drives Engagement:** There is a direct correlation between the emotional intensity of a post and public interaction metrics (likes, comments, views, and retweets). 
* **Partisan Differences:** Compared to Democrats, Republicans tend to use more intense language on average. Furthermore, the public shows a higher sensitivity (stronger reaction) to intense language when used by Republican politicians.
...
* **`models/`**: Includes the RoBERTa model used for intensity labeling and the STM model files.
* **`plots/`**: Contains all visualizations, including the STM analysis plots.
* **`scripts/`**: The R and Python scripts used for the analysis.
* **`data/`**: (External) This folder contains the raw and processed data (see below).

---

## 💾 Data Access
Due to the size of the datasets, the `data/` folder is not included in this repository. 

**[Download the Data Folder Here](https://drive.google.com/drive/folders/1XdZzdvmNVOb0iOYJAYp_WI833BdHpMyS?usp=sharing)**
...
To replicate this research, please run the scripts in the `scripts/` folder according to their numerical prefix:
